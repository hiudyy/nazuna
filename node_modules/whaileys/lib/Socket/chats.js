...
const makeChatsSocket = (config) => {
    const { logger, markOnlineOnConnect, shouldSyncHistoryMessage, shouldIgnoreJid, fireInitQueries } = config;
    ...
    const onWhatsApp = async (...jids) => {
        const results = await interactiveQuery(jids.map(jid => ({
            tag: "user",
            attrs: {},
            content: [
                {
        ...
        })), { tag: "contact", attrs: {} }, { tag: "lid", attrs: {} });
        return results
            .map(user => {
            const contact = (0, WABinary_1.getBinaryNodeChild)(user, "contact");
            const lid = (0, WABinary_1.getBinaryNodeChild)(user, "lid");
            return {
                exists: (contact === null || contact === void 0 ? void 0 : contact.attrs.type) === "in",
                jid: user.attrs.jid,
                lid: lid === null || lid === void 0 ? void 0 : lid.attrs.val
            };
        })
    ...
    const updateProfilePicture = async (jid, content) => {
        const targetJid = (0, WABinary_1.jidNormalizedUser)(jid) !== (0, WABinary_1.jidNormalizedUser)(authState.creds.me.id)
    ...
    const getBusinessProfile = async (jid) => {
        ...
        if (profiles) {
            ...
            const business_hours_config = business_hours &&
                (0, WABinary_1.getBinaryNodeChildren)(business_hours, "business_hours_config");
    ...
    const resyncAppState = ev.createBufferedFunction(async (collections, isInitialSync) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            while (collectionsToHandle.size) {
                ...
                const decoded = await (0, Utils_1.extractSyncdPatches)(result, config === null || config === void 0 ? void 0 : config.options); // extract from binary node
                for (const key in decoded) {
                    ...
                    try {
                        ...
                        if (patches.length) {
                            const { newMutations, state: newState } = await (0, Utils_1.decodePatches)(name, patches, states[name], getAppStateSyncKey, onMutation, config.options, initialVersionMap[name]);
                            await authState.keys.set({
                                "app-state-sync-version": { [name]: newState }
                            });
                        ...
                        logger.info({ name, error: error.stack }, `failed to sync state from version${isIrrecoverableError
                            ? ""
    ...
    const appPatch = async (patchCreate) => {
        ...
        if (config.emitOwnEvents) {
            ...
            ], initial, getAppStateSyncKey, onMutation, config.options, undefined, logger);
        }
    };
    ...
    const upsertMessage = ev.createBufferedFunction(async (msg, type) => {
        ...
        if (!!msg.pushName) {
            ...
            if (msg.key.fromMe &&
                msg.pushName &&
                ((_a = authState.creds.me) === null || _a === void 0 ? void 0 : _a.name) !== msg.pushName) {
