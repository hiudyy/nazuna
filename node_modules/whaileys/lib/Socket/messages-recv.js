...
const makeMessagesRecvSocket = (config) => {
    ...
    const { ev, authState, ws, query, processingMutex, upsertMessage, resyncAppState, onUnexpectedError, assertSessions, sendNode, relayMessage, sendReceipt, uploadPreKeys, sendPeerDataOperationMessage } = sock;
    ...
    const sendMessagesAgain = async (key, ids, retryNode) => {
        var _a;
        const msgs = await Promise.all(ids.map(id => (sentMessagesCache === null || sentMessagesCache === void 0 ? void 0 : sentMessagesCache.get(id)) ||
            getMessage({ ...key, id })));
                ...
                processingMutex.mutex(async () => {
                    ...
                    if (typeof status !== "undefined" &&
                        ...
                        if ((0, WABinary_1.isJidGroup)(remoteJid)) {
                            if (attrs.participant) {
                                const updateKey = status === WAProto_1.proto.WebMessageInfo.Status.DELIVERY_ACK
                                    ? "receiptTimestamp"
                                    : "readTimestamp";
    ...
    const handleMessage = async (node) => {
        if (shouldIgnoreJid(node.attrs.from) &&
            ...
            await Promise.all([
                processingMutex.mutex(async () => {
                    ...
                    if (msg.messageStubType === WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT &&
                        category !== "peer") {
                        logger.debug({ key: msg.key, params: msg.messageStubParameters }, "failure in decrypting message");
    ...
    const requestPlaceholderResend = async (messageKeys) => {
        var _a;
        if (!((_a = authState.creds.me) === null || _a === void 0 ? void 0 : _a.id))
            throw new boom_1.Boom("Not authenticated");
        const pdoMessage = {
